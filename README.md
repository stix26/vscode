...
These packages match the setup used in the project's CI configuration and must be installed **before** running `npm ci`. If they are missing, native modules can fail to build with errors such as `gssapi/gssapi.h: No such file or directory`, which will cause pre-commit hooks or local builds to fail.
